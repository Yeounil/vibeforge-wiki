...
export function MobileStickyTOC({ title, headings, containerRef }: Props) {
  ...
  return (
    <>
      {/* Sticky top bar. Kept SOLID (no backdrop-filter) — backdrop-filter
       *  on an ancestor of a position:fixed child creates a CSS containing
       *  block, which would re-anchor the dialog/backdrop below to this nav
       *  instead of the viewport. */}
      <nav
        aria-label="목차 (모바일)"
        className="lg:hidden sticky top-0 z-30 -mx-6 sm:-mx-8 mb-4 bg-[var(--canvas)] border-b border-[var(--hairline)]"
        style={{ height: BAR_H }}
      >
        <button
          type="button"
      ...
      {/* Dialog + backdrop rendered as siblings of <nav>, NOT children, so
       *  position:fixed bottom:0 anchors to the viewport. */}
      {open && (
        <>
          <button
            type="button"
            ...
            className="lg:hidden fixed inset-0 z-40 bg-[var(--overlay-scrim)]"
          ...
          <div
            ...
            aria-modal="true"
            aria-label="목차"
            className="lg:hidden fixed left-0 right-0 bottom-0 z-50 max-h-[60vh] overflow-y-auto bg-[var(--canvas)] border-t border-[var(--hairline)] rounded-t-2xl shadow-2xl pb-[env(safe-area-inset-bottom)]"
          >
