...
test.describe("visual shell", () => {
  test("homepage hero shows VibeForge title and CTA", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /위키 둘러보기/ })).toBeVisible();
  });
  ...
  test("web font is applied to body", async ({ page }) => {
    await page.goto("/");
    const family = await page.evaluate(() =>
      window.getComputedStyle(document.body).fontFamily
    );
    expect(family).toMatch(/__pretendard|pretendard|__geist|geist/i);
  });
});
